...
public:
   ...
   Motion ComputeArmFreeze(const Motion& motion) {
      ...
      quat elbowRRot = eulerAngleRO(XYZ, radians(vec3(0, -23, 0)));
      quat elbowLRot = eulerAngleRO(XYZ, radians(vec3(0, 23, 0)));
      ...
      vec3 ballPos = leftArm->getGlobalTranslation();
      ...
      for (int i = 0; i < motion.getNumKeys(); i++){
         ...
         pose.jointRots[leftElbowID] = elbowRRot;
         pose.jointRots[rightElbowID] = elbowLRot;
         result.appendKey(pose);
      }
      return result;
   }
   ...
   void scene() {  
      ...
      i += dt() * 0.245;
      ballPos = vec3(0, 150, 70) *(1.0f - i) + vec3(0,150,370)*i;
      setColor(vec3(1,0,0));
      drawSphere(ballPos, ballSize);
   }
